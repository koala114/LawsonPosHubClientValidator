...
import com.kargotest.alading.ISVALaDingHandler;
import com.kargotest.fft.ISVFFTHandler;
import com.kargotest.kargo.ISVKargoHandler;
import com.kargotest.ebuy.ISVeBuyHandler;
import com.kargotest.miya.ISVMiYaHandler;
import com.kargotest.miya.MiYa;
import com.kargotest.miya.DuoDian;
import com.kargotest.YoRen.YoRen;
import com.kargotest.xinli.ISVXinLiHandler;
import com.kargotest.zhongbai.ZhongBaiISVHandler;
...
public class HTTPServer {
    ...
    public static void main(String[] args) {
        try {
            ...
            host.addContext("/miya/api/pay/", new MiYa(), "POST");
            host.addContext("/miya/miyapay_response.action", new ISVMiYaHandler(), "POST");
            ...
            host.addContext("/DLLCashier", new ISVALaDingHandler(), "POST");
            host.addContext("/mssp/odsi/pay/service", new ZhongBaiISVHandler(), "POST");
            host.addContext("/oap-gw/gateway.do", new ISVFFTHandler(), "POST");


            host.addContext("/api/payments/v2.0/pay", new ContextHandler() {
                @Override
                public int serve(Request req, Response resp) throws IOException {
                    resp.getHeaders().add("Content-Type", "application/json");
                    resp.send(200, "{\"merchantTransactionID\":\"208888171330168192\",\"kargoCardTransactionID\":\"00103267070G\",\"hostTransactionID\":\"82230517486194240761\",\"responseCode\":\"0000\",\"responseMessage\":\"Txn completed successfully\",\"accountInfo\":\"WALLET:UNIPAY\",\"amountRequested\":17.56,\"amountProcessed\":17.56,\"primaryAccountNumber\":\"6221373301232066438\",\"prepaidBalance\":17.56}");
                    return 0;
                }
            }, "POST");

            /*host.addContext("/rps/gift/server/api/program-pyt-pp/v1.0/redeemprepaid", new ContextHandler() {
                @Override
                public int serve(Request req, Response resp) throws IOException {
                    resp.getHeaders().add("Content-Type", "application/json");
                    resp.send(200, "{\"merchantTransactionID\":\"208888171345349072\",\"kargoCardTransactionID\":\"001032669712\",\"hostTransactionID\":\"4200001025202305171345386576\",\"responseCode\":\"0000\",\"reasonCode\":\"0000\",\"responseMessage\":\"Txn completed successfully\",\"prepaidBalance\":0.66,\"accountStatus\":\"WALLET:WCTPAY\",\"amountRequested\":0.66,\"amountProcessed\":0.66,\"walletUserAccount\":\"obXJst1S2WxdhngjKt1T7ui1fvDs\",\"primaryAccountNumber\":\"133072400089601371\",\"walletPayCompleteTime\":\"20230517134538\"}");
                    resp.send(200, "{\"merchantTransactionID\":\"208888241104539802\",\"kargoCardTransactionID\":\"001033916212\",\"hostTransactionID\":\"001033916212\",\"responseCode\":\"9000\",\"reasonCode\":\"9065\",\"responseMessage\":\"Insufficient account funds\",\"prepaidBalance\":0.0,\"accountStatus\":\"WALLET:WCTPAY\",\"amountRequested\":0.01,\"amountProcessed\":0.01,\"primaryAccountNumber\":\"134280065829410384\"}");
                    return 0;
                }
            }, "POST");*/


            host.addContext("/api/v2/health", new ContextHandler() {
                @Override
                public int serve(Request req, Response resp) throws IOException {
                    resp.getHeaders().add("Content-Type", "application/json");
                    resp.send(200, "{\"responseCode\":\"0000\",\"responseMessage\":\"Server is up and running\",\"ret_code\":\"00\",\"ret_msg\":\"Server is up and running\"}");
                    return 0;
                }
            }, "GET");

            host.addContext("/api/wallet/uploadgoodsdetail", new ContextHandler() {
                @Override
                public int serve(Request req, Response resp) throws IOException {
                    resp.getHeaders().add("Content-Type", "application/json");
                    resp.send(200, "{\"responseCode\":\"0000\",\"responseMessage\":\"Txn completed successfully\",\"companyId\":\"01\",\"settleCompayId\":\"004\",\"processTime\":74,\"storeId\":\"208888\",\"biz_content\":{\"ret_code\":\"00\",\"ret_msg\":\"请求成功[商品明细上传]\",\"out_trade_no\":\"20888813110953052\",\"pay_id\":\"038\",\"pay_code\":\"038\",\"pay_name\":\"罗森点点\"}}");
                    return 0;
                }
            }, "POST");
